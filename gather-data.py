        ...
        while count < max_tweets:
            ...
            if max_id == -1:
                statuses = twitter_api.GetSearch(
                    term=search_query, locale = 'eu', count=100, result_type='mixed')
            else:
                statuses = twitter_api.GetSearch(
                    term=search_query, locale = 'eu', count=100, result_type='mixed', max_id=max_id - 1)
...
def main(twitter_handle):
    global twitter_api
    twitter_api = twitter.Api(consumer_key= '<INSERT_TWITTER_CONSUMER_KEY_HERE>',
